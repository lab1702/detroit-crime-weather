...
def pfmt(p):
    """Readable p-value: 'p < 0.001' in the tail, two/three sig-figs otherwise."""
    if p < 0.001:
        return 'p&nbsp;&lt;&nbsp;0.001'
...
# means the log-linear temperature response is not masking real curvature. The
# leading adjective (shape_adj) also adapts, so we never assert "close to linear"
# in the same breath as conceding significant curvature.
_curv_sig = tot_curv_p < 0.05
shape_adj = 'close to linear' if not _curv_sig else 'broadly monotonic'
linear_phrase = ('and the log-linear fit holds: a quadratic temperature term '
    ...
    'though a quadratic term detects mild curvature, so the per-10&deg;F figure '
...
<p class="lead">Across {ndays:,} days, the daily mean temperature tracks about <b>{tot_r**2*100:.0f}% of the variance in daily crime counts</b> (Pearson r&nbsp;=&nbsp;{tot_r:.2f}) &mdash; though most of that is the shared seasonal swing, not genuine day-to-day movement. Stripped of the calendar, the link is weaker but real: warmer-<i>than-normal</i> days still carry more crime than their season predicts (about <b>{tot_anom**2*100:.0f}% of the within-season variance</b>, r&nbsp;=&nbsp;{tot_anom:.2f}; section&nbsp;04). The relationship is strong, positive, and {shape_adj} ({linear_phrase}): each additional 10&deg;F is associated with roughly <b>{tot_pct:.0f}% more reported crime</b>.</p>
...
<figure><img src="{b64('figs/fig3_sensitivity.png')}" alt="Temperature sensitivity by category"><figcaption>Slope of daily incidents on temperature, scaled to each category's own average. Several categories show detectable curvature, so for those the bar is the average slope across the observed temperature range rather than a constant rate at every temperature. Bar length is the raw per-10&deg;F slope; <b>solid</b> bars survive deseasonalizing (the within-season test of section&nbsp;04), while <b>faded</b> bars do not — "seasonal" marks an effect that is detectable overall but not within season, and "n.s." one that is not significant at all.</figcaption></figure>
...
<p>It doesn't. Warmer-than-normal days still carry more crime than normal (r&nbsp;=&nbsp;{tot_anom:.2f}), and the effect is strongest for the high-volume violent categories flagged above (aggravated assault, assault, weapons, homicide). A few lower-volume offenses whose raw temperature slope looked steep &mdash; sexual assault, disorderly conduct, OUIL &mdash; lose significance once the season is stripped out, a reminder that the per-category &ldquo;sensitivity&rdquo; in section&nbsp;03 still carries seasonal confounding. But for the violent core, an unseasonably hot day is a higher-crime day on its own.</p>
...
<figure><img src="{b64('figs/fig13_rainsnow.png')}" alt="Rain vs snow effect by crime type"><figcaption>Regression coefficients per inch of precipitation, controlling for daily mean temperature. "n.s." = not significant. Note the two scales differ physically: rain is liquid depth (a typical wet day &asymp;&nbsp;0.09&nbsp;in) while snow is snow depth (&asymp;&nbsp;7&times; the water equivalent; a typical snow day &asymp;&nbsp;0.5&nbsp;in), so a full inch extrapolates farther into the tail for rain than for snow &mdash; read each bar against its own typical event, not against the other.</figcaption></figure>
...
<p>Wind tells the same story as rain, and tells it independently. Holding both temperature and precipitation fixed, each extra <b>10&nbsp;mph of peak wind</b> shaves about <b>{abs(wind_v_eff):.1f} violent incidents</b> off the day ({pfmt(wind_v_p)}) — yet leaves property crime essentially untouched ({wind_p_eff:+.1f}/day, n.s.). A blustery day is an uncomfortable day to be loitering on a corner.</p>
...
<p>It does not. Comparing day&nbsp;3+ of a heat wave to an isolated hot day of the same temperature, the difference is a statistically insignificant {hw_v_eff:+.1f} violent incidents ({pfmt(hw_v_p)}) — and likewise flat for total crime. Crime rises with the thermometer each day and resets with it; the heat does not "bank."</p>
